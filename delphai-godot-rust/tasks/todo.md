...
- [x] `resource.rs` 新規: `Resource { kind: Berry, amount, tile_pos }` と再生ロジック
- [x] `agent/behavior.rs` 新規: `BehaviorState { Idle, SeekingFood, Gathering }`、純粋関数 `decide(state, vitals, tile, nearest_berry) -> (state, BehaviorAction)`
- [x] `world.rs` に `Vitals { fed }` と `FED_DECAY=0.004/tick`、`FED_LOW=0.4` 閾値、`FED_GAIN_PER_GATHER=0.2`
- [x] gdext FFI: `set_berry_tiles(PackedInt32Array)` / `get_resource_count` / `get_resource_world_pos` / `get_resource_amount` / `get_citizen_fed`
- [x] GDScript: `BERRY_TILES` 3 個、red SphereMesh 描画、amount で scale + 灰 lerp フェード
- [x] RED → GREEN（TDD、core 154 / gdext 13 pass）
- [x] **検証 (3 点)**: `cargo test --workspace` 154+13 pass / `cargo clippy -- -D warnings` 0 warn / `make build` ✅（人間目視は Mac で実施）

#### N6 初日の地雷

1. **tick 順序は「decay → decide → step → history → regenerate → random_walk」**。drain を decide より後にすると閾値越えの tick で 1 tick 分 Idle のまま待つ羽目になる。
2. **Gather 時に `move_target = None` を打つ**。target 残ったままだと citizen が berry を通り過ぎて振動する。`step_with_grid` は target クリアを呼び出し側依存。
3. **random_walk は `BehaviorState::Idle` に限定**する。Gathering/SeekingFood を random_walk が横取りすると「茂みに着いた瞬間よそに走り去る」バグが出る。
4. **`nearest_non_depleted_berry` は squared Euclidean on `i32`**（i16 で差分二乗は overflow する）。depleted は min_by_key 候補から除外（空の最寄り bush に引き寄せられて餓死するのを防ぐ）。
5. **`Vitals` と `citizen_behaviors` は `spawn_citizen` で parallel-array push**。独立 push 漏れがあると `tick` が panic（`citizens.len() != citizen_vitals.len()`）。
6. **FFI の berry seed は `PackedInt32Array` 偶数長**。奇数個は末尾捨てて clamped、Godot 側 `[x0,y0,x1,y1,...]` と合意。
7. **GDScript の `_update_berry_visuals` は毎フレーム呼ぶ**（scale/color の連続フィードバック）。tick-accum ブロック内だと 4Hz でカクつく。
8. **gather量のクランプ**: `fed = min(1.0, fed + FED_GAIN_PER_GATHER * taken)`。満腹後も gather し続けると次 tick で Idle 遷移が正、taken=0（bush 空）でも fed 据え置き。
...
**導入完了**: `CoplayDev/unity-mcp` v9.6.6 を Mac の Unity Editor に UPM 経由で導入、HTTP transport (port 8080) で `claude` から接続成功。スモークテスト pass（Hierarchy 読み取り + `SmokeTestCube` 追加 + コンソール error 0）。Godot 側 `cargo test --workspace` 154+13 passed で Don't burn the bridge も green 維持。

**起動手順**（次回以降）:
1. Mac で Unity Editor を開く（プロジェクトルート `/workspaces/delphai`）
2. Window > Unity MCP > Start Server をクリック → port 8080 で listen
3. プロジェクトルートで `claude` 起動 → ルート `.mcp.json` の `unityMCP` エントリ経由で自動接続、`/mcp` GREEN を確認
4. 動作確認: `manage_scene action=get_active` で空シーン取得、`read_console` で error/warning 0

**既知の制約**:
- Editor 起動 + Start Server を**毎回手動**でクリック必須（自動起動できない）
- `make build` / Godot 動作確認は devcontainer 不可、Mac ローカル責務
- M0.5 はスモークテスト範囲（読み取り中心 + Cube 1 個）に限定。M1 以降のシーン編集は破壊リスクがあるためコミット粒度を細かく
- stdio fallback を使う場合は `uv` 要インストール、`.mcp.json` の `_unityMCPStdio_disabled` を有効化（今回は HTTP で完結、未使用）
