...
## Unity 移行 — Phase M0.5 メモ（2026-04-26）

- 採用 MCP: `CoplayDev/unity-mcp` v9.6.6（justinpbarnett 側は CoplayDev に統合済）
- UPM git URL: `https://github.com/CoplayDev/unity-mcp.git?path=/MCPForUnity#main`（Editor の Window > Package Manager）
- 接続: 既定 HTTP `http://localhost:8080/mcp`、Editor で Window > Unity MCP > Start Server を押す → ルート `.mcp.json` 経由で `claude` 起動時に自動接続
- Fallback: stdio 経由は `uv` (Python 3.10+) が必要、`.mcp.json` の `_unityMCPStdio_disabled` を有効化して切り替え
- 既知制約: M0.5 は読み取り中心 + Cube 1 個までに絞る（シーン破壊リスク）。本格運用は M1 以降、コミット粒度を細かく
